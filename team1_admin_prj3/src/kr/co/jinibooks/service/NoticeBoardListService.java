...
public class NoticeBoardListService {
	 ...
	 * @return
	 */
	public int totalCount(NoticeSearchVO sv) {
		int totalCnt = 0;
...
//		    }//end if 
	...
	// 현재 게시판의 페이지 인덱스 설정
	public String indexList(IndexListVO ilVO,NoticeSearchVO sVO) {
	...
	public boolean insertnotice2(NoticeVO ceiVO){
		boolean flag = false;
		...
		try {
			//쿼리문을 수행하고 결과를 얻는다.
			int cnt = ceDAO.insertNotice2(ceiVO);
			if(cnt == 1) {
				flag = true;
			}//end if
		} catch (SQLException sqle) {
